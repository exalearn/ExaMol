...
def test_multifi_loader(training_set, scorer, multifi_recipes):
    ...
    # Test the loader
    outputs = collect_outputs(training_set, multifi_recipes)
    loader = make_data_loader(inputs, outputs.tolist(), batch_size=1, value_spec=tf.TensorSpec((len(multifi_recipes),), dtype=tf.float32))
...
def test_multifi_model(atomwise, training_set, multifi_recipes, scorer):
    # Make the network
    model = make_simple_network(atom_features=8, message_steps=4, output_layers=[32, 16], outputs=len(multifi_recipes))
    assert model.output_shape == (None, len(multifi_recipes))
    lower_fidelities = collect_outputs(training_set, multifi_recipes[:-1])

    # Train it
    parsed_inputs = scorer.transform_inputs(training_set)
    parsed_outputs = scorer.transform_outputs(training_set, multifi_recipes[-1])
    model_msg = scorer.prepare_message(model, training=True)

    update_msg = scorer.retrain(model_msg, parsed_inputs, parsed_outputs, lower_fidelities=lower_fidelities, timeout=1, batch_size=1)
